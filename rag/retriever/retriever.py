...
class Retriever:
    ...
    def _get_retrieved_lists(
        ...
        # 分别检索三种检索方法
        dense_retrieved_list = dense_search(query_embedding=query_embedding["dense"][0], topk=self.topk * 5)
        sparse_retrieved_list = sparse_search(query_embedding=query_embedding["sparse"], topk=self.topk * 2)
        # 不确定检索时间消耗，并行反而可能减速
    ...
    def retrieve(self, query_list: List[str]) -> List[List[str]]:
        retrieved_docs = []
        query_embeddings = self.model(query_list)
