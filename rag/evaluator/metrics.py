...
class BaseMetric:
    ...
    # 假设 data 是一个列表，每个元素是一个包含 'choices' 和 'golden_answers' 的字典
    # 这里需要根据实际的数据结构进行调整
        golden_answers_list = []
        for item in data:
            if 'choices' in item and 'golden_answers' in item:
                if any(choice == [] for choice in item['choices']):
              ...
              # 如果数据项不包含所需的键，可以抛出错误或进行其他处理
              raise ValueError("Data item is missing 'choices' or 'golden_answers' key.")
    ...
    def token_level_scores(self, prediction: str, ground_truths: str):
        ...
        for ground_truth in ground_truths:
            ...
            if (
                ...
                continue
            prediction_tokens = set(jieba.cut(normalized_prediction))
            ground_truth_tokens = set(jieba.cut(normalized_ground_truth))
...
class Rouge_Score(BaseMetric):
    ...
    def calculate_rouge(self, pred, golden_answers):
        ...
        for answer in golden_answers:
            ...
            scores = self.scorer.get_scores(pred_cut, answer_cut)
